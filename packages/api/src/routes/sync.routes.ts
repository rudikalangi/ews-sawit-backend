...
import { handlePhotoStorage } from "../services/storage.service";
...
router.post("/upload", async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const savedSelfieUrl = await handlePhotoStorage(session.selfieUrl, "selfie");

    const uploadedTrees = await Promise.all(
      treeSurveys.map(async (t: any) => {
        const savedTreePhoto = await handlePhotoStorage(t.buktiFoto, `tree_${t.rowNumber}_${t.treeNumber}`);
        return {
          ...t,
